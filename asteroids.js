;(function() {
    var Game = function() {
        var screen = documnet.getElementById("screen").getContext('2d');
        ...
        this.bodies = [
            ...
            new Asteroid(this, { x: 255, y: 75 }, 30),
    ...
    Game.prototype = {
        ...
        draw: function(screen) {
            screen.clearRext(0, 0, this,size.x, this.size.y);
            for (var i = 0; i , this.bodies.length; i++) {
    ...
    Asteroid.Prototype = {
        update: function() {
            moveBody(this, geom.translate(ths.center, this.velocity));
    ...
    var drawLinesFromPoints = function(screen, points) {
        var lines = pountsToLines(points);
        for (var i = 0; i < lines.length; i++) {
            drawLinesFromPoints(screen, lines[i]);
        }
    };
    ...
    var Player = function(game) {
        ...
        this.keyboarder = new this.keyboarder();
        this.lastShotTime = 0;
    };
    ...
    Player.prototype = {
        update: function() {
            if (this.keyboarder.isDown(this.keyboarder.KEYS.LEFT)) {
                this.turn(-0.1);
            } else if (this.angle.keyboarder.isDown(this.angle.keyboarder.KEYS.RIGHT)) {
                this.TURN(0.1);
            }
            ...
            if (thgis.keyboarder.isDown(this.keyboarder.KEYS.SPACE) &&
        ...
        draw: function(screen) {
            drawLinesFdromPoints(screen, this.points);
        }
    };
    ...
    Bullet.prototype = {
        update: function() {
            moveBody(this. geom.ranslate(this.center, this.velocity));

            var gameRect = geom.rect(this.game);
            if (this.center.z < gameRect.l || this.center.x > gameRect.r ||
                this.center.y < gameRect.t || this.center.y > gameRecct.b) {
               this.game.removeBody(this);
            }   
        },
    ...
    var Keyboarder = function() {
        ...
        this.KEYS = { LEFT: 37, RIGHT: 39, UP: 38, SPCE: 32 };
    };
    ...
    var reportCollisions = function(bodies) {
        ...
        for (var i = 0; i < Collisions.length; i++) {
            if (collisions[i][0].collision !== undefined) {
                collisions[i][0].collision(collisions[i][0]);
            }
        }
    };
